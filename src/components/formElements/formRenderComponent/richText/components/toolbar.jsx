...
const Toolbar = ({ editor, onUpload, isUploading, isFullscreen, onToggleFullscreen }) => {
  ...
  const headingItems = HEADINGS.map((item) => ({
    key: item.value,
    label: item.label,
    onClick: () => {
    ...
    <div className={`${styles.toolbar} ${isFullscreen ? styles.fullscreenToolbar : ''}`}>
      <Space wrap size={2}>
        ...
        <MenuButton
          ...
          title={formatPainterActive ? '格式刷已激活(选中文本应用格式)' : '格式刷(先选中带格式文本再点击)'}
      ...
      <Modal
        ...
        footer={[
          <Button key="remove" danger onClick={removeLink}>移除链接</Button>,
          <Button key="cancel" onClick={() => setLinkModalOpen(false)}>取消</Button>,
          <Button key="submit" type="primary" onClick={confirmLink}>确定</Button>,
        ]}
      >
        <Input
          placeholder="请输入链接地址"
